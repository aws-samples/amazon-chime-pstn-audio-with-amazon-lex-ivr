...
const SipProvider = ({ children }) => {
    ...
    const [lexInfo, setLexInfo] = useState(null);
    ...
    const onInvite = useCallback((invitation) => {
        ...
        const xLexinfoHeader = incomingHeaders.find((header) => 'X-Lexinfo' in header);

        if (incomingHeaders.find((header) => 'X-Lexinfo' in header)) {
            setLexInfo(xLexinfoHeader['X-Lexinfo']);
        } else {
            setLexInfo(null);
        }
